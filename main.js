...
function createTray() {
  ...
  tray.setToolTip("Lazy Print");

  const menu = Menu.buildFromTemplate([
    {
      label: "Tampilkan Lazy Print",
      click: () => { mainWindow.show(); mainWindow.focus(); },
    },
    { type: "separator" },
    {
      label: "Tambah File...",
      click: () => { mainWindow.show(); mainWindow.webContents.send("tray-open-files"); },
    },
    {
      label: "Mulai Print",
      click: () => { mainWindow.show(); mainWindow.webContents.send("tray-start-print"); },
    },
    { type: "separator" },
    {
      label: "Keluar",
...
const PAPER_DIMS = {
  a4:     { w: 595,  h: 842,  word: 8  },
  a3:     { w: 842,  h: 1190, word: 7  },
  a5:     { w: 420,  h: 595,  word: 11 },
  letter: { w: 612,  h: 792,  word: 1  },
  legal:  { w: 612,  h: 1008, word: 5  },
  b5:     { w: 499,  h: 708,  word: 13 },
  f4:     { w: 595,  h: 935,  word: 12 },
};
...
$n.Dispose()`;
  ...
  try {
    ...
    exec(
      'powershell -NoProfile -NonInteractive -WindowStyle Hidden -ExecutionPolicy Bypass -File "' +
        tmpFile +
        '"',
      { timeout: 8000 },
      () => {
    ...
    if (gsExe) {
      try {
        const result = await runCmd(
          `"${gsExe}" -dBATCH -dNOPAUSE -dNOSAFER -sDEVICE=nullpage -dQUIET "${filePath}"`,
          30000,
          true,
        );
        const matches = (result.stderr || "").match(/Page\s+(\d+)/g);
        if (matches && matches.length > 0)
          return parseInt(matches[matches.length - 1].replace(/\D/g, ""));
      } catch {}
    }
...
ipcMain.handle("get-preview", async (event, { filePath, page }) => {
  ...
  if (ext === ".pdf") {
    const gsExe = findGhostscript();
    if (!gsExe)
      return { type: "error", msg: "Install Ghostscript untuk preview PDF" };
    ...
    try {
      ...
      if (!fs.existsSync(outFile))
        return { type: "error", msg: "Ghostscript gagal render halaman ini" };
  ...
  if (ext === ".txt") {
    try {
      ...
      return {
        type: "html",
        content: `<!DOCTYPE html><html><body style="margin:16px;font-family:Consolas,monospace;font-size:11px;background:#1c1c28;color:#e8e8f0;white-space:pre-wrap;word-break:break-word;line-height:1.5">${escaped}</body></html>`,
        totalPages,
      };
    } catch (e) {
      return { type: "error", msg: e.message };
  ...
  if ([".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx", ".odt", ".ods", ".rtf", ".html", ".htm"].includes(ext)) {
...
try {
  ...
  $pr.Close()
      ...
      /* MS Office tidak ada, coba LibreOffice */
    }
    ...
    if (!msSuccess && libreExe) {
      try {
        ...
        await runCmd(
          `"${libreExe}" --headless --convert-to pdf --outdir "${outDir}" "${filePath}"`,
          60000,
        );
    ...
    if (!msSuccess) {
      ...
      const hint = libreExe
        ? ""
        : " Install Microsoft Office atau LibreOffice (gratis).";
      return {
        type: "error",
        msg: `Preview gagal. Tidak ada Office/LibreOffice terdeteksi.${hint}`,
      };
    }
    ...
    if (!gsExe) {
      ...
      return {
        type: "error",
        msg: "Install Ghostscript untuk render preview.",
      };
    }
  ...
  return {
    type: "error",
    msg: `Format .${ext.replace(".", "")} tidak mendukung preview`,
  };
});
...
async function pdfPageToPng(gsExe, pdfPath, page) {
  ...
  try {
    ...
    if (!fs.existsSync(outFile))
      return { type: "error", msg: "Ghostscript gagal render" };
    const b64 = fs.readFileSync(outFile).toString("base64");
    fs.unlink(outFile, () => {});
    return { type: "image", data: `data:image/png;base64,${b64}` };
  } catch (e) {
...
// PRINT TO PDF
// ============================================================
// Helper: convert any file to PDF via LibreOffice headless
async function convertWithLibre(filePath, tmpPdf, libreExe) {
  const ext = path.extname(filePath).toLowerCase();
  const tmpDir = path.dirname(tmpPdf);
  const base = path.basename(filePath, ext);
  await runCmd(
    `"${libreExe}" --headless --convert-to pdf --outdir "${tmpDir}" "${filePath}"`,
    120000,
  );
  const loOut = path.join(tmpDir, base + ".pdf");
  if (fs.existsSync(loOut)) {
    fs.renameSync(loOut, tmpPdf);
    return true;
  }
  return false;
}
  ...
  async (event, { files, outputPath, pageFrom, pageTo }) => {
    ...
    try {
      for (const filePath of files) {
        ...
        try {
          if ([".doc", ".docx"].includes(ext)) {
            const ok = libreExe && await convertWithLibre(filePath, tmpPdf, libreExe);
            if (!ok) await convertOfficeToPdf(filePath, tmpPdf, "word");
          } else if ([".xls", ".xlsx"].includes(ext)) {
            const ok = libreExe && await convertWithLibre(filePath, tmpPdf, libreExe);
            if (!ok) await convertOfficeToPdf(filePath, tmpPdf, "excel");
          } else if ([".ppt", ".pptx"].includes(ext)) {
            const ok = libreExe && await convertWithLibre(filePath, tmpPdf, libreExe);
            if (!ok) await convertOfficeToPdf(filePath, tmpPdf, "ppt");
          } else if (ext === ".txt") {
            const ok = libreExe && await convertWithLibre(filePath, tmpPdf, libreExe);
            if (!ok) await convertTxtToPdf(filePath, tmpPdf);
          } else if ([".odt", ".ods", ".rtf", ".html", ".htm"].includes(ext)) {
            if (libreExe) await convertWithLibre(filePath, tmpPdf, libreExe);
          } else if ([".png", ".jpg", ".jpeg", ".bmp"].includes(ext)) {
            await runCmd(
              `"${gsExe}" -dBATCH -dNOPAUSE -dNOSAFER -sDEVICE=pdfwrite -sOutputFile="${tmpPdf}" "${filePath}"`,
              30000,
            );
          }
          ...
          console.log("Skip convert error:", e.message);
        }
      }
      if (!pdfFiles.length)
        throw new Error("Tidak ada file yang berhasil dikonversi.");
      const pageFlags =
        pageFrom && pageTo
...
ipcMain.handle("get-lifetime-stats", async () => {
  const h = loadHistory();
  const success = h.filter((x) => x.success === true).length;
  const failed = h.filter((x) => x.success === false).length;
  return { total: h.length, success, failed };
});
...
ipcMain.handle("save-queue", async (event, files) => {
  ...
  try {
    fs.writeFileSync(queueFile, JSON.stringify(slim, null, 2), "utf8");
  } catch {}
  return slim;
});
...
app.on("before-quit", () => {
  Object.values(watcherMap).forEach((w) => {
    try {
      w.close();
    } catch {}
  });
...
ipcMain.handle(
  "print-files",
  async (event, { files, printer, copies, orientation, nup, duplex, paperSize }) => {
    const results = [];
    for (const item of files) {
      ...
      try {
        ...
        results.push({ file: filePath, success: false, error: wrapPrintError(ext, err.message) });
      }
    }
    return results;
  },
...
async function printFileSilent(
  ...
  if (process.platform === "win32") {
    ...
    if ([".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx"].includes(ext)) {
      // LibreOffice dulu — convert ke PDF → Ghostscript, tidak ada app yang terbuka
      // Fallback ke MS Office COM jika LibreOffice tidak terinstall
      const libreExe = findLibreOffice();
      if (libreExe)
        return printOfficeLibre(filePath, printer, copies, orientation, paperSize, pageFrom, pageTo, nup, duplex);
      // MS Office COM fallback
      if ([".doc", ".docx"].includes(ext))
        return printWordSilent(filePath, printer, copies, orientation, pageFrom, pageTo, duplex, paperSize);
      if ([".xls", ".xlsx"].includes(ext))
        return printExcelSilent(filePath, printer, copies, duplex);
      if ([".ppt", ".pptx"].includes(ext))
        return printPptSilent(filePath, printer, copies);
      throw new Error(`Install LibreOffice (libreoffice.org) atau Microsoft Office untuk print ${ext}.`);
    }
    if (ext === ".txt") return printTxtSilent(filePath, printer, copies);
    if ([".png", ".jpg", ".jpeg", ".bmp"].includes(ext))
      return printImageSilent(filePath, printer, copies);
    if ([".odt", ".ods", ".rtf", ".html", ".htm"].includes(ext))
      return printOfficeLibre(filePath, printer, copies, orientation, paperSize, pageFrom, pageTo, nup, duplex);
    ...
    return runCmd(
      `lp -d "${printer}" -n ${copies} ${orientFlag} ${pageFlag} ${nupFlag} ${duplexFlag} "${filePath}"`,
    );
  }
}
...
function printPdfGS(
  ...
  if (orientation === "landscape") {
    orientFlag = `-dFIXEDMEDIA -dDEVICEWIDTHPOINTS=${paper.h} -dDEVICEHEIGHTPOINTS=${paper.w}`;
  } else if (paperSize && paperSize !== "a4") {
    orientFlag = `-dFIXEDMEDIA -dDEVICEWIDTHPOINTS=${paper.w} -dDEVICEHEIGHTPOINTS=${paper.h}`;
  }
  const pageFlags =
    pageFrom && pageTo ? `-dFirstPage=${pageFrom} -dLastPage=${pageTo}` : "";
  // N-up: use Ghostscript pstops-style via nup device
  const nupFlag =
    nup > 1 ? `-dNupW=${nup <= 2 ? 2 : 2} -dNupH=${nup <= 2 ? 1 : 2}` : "";
  ...
  const cmd = [
    ...
    nupFlag,
    duplexFlag,
    `-sOutputFile="%printer%${printer.replace(/"/g, '\\"')}"`,
    `"${filePath}"`,
  ]
    .filter(Boolean)
    .join(" ");
  exec(cmd, { timeout: 120000 }, (err, stdout, stderr) => {
    if (
      ...
      reject(new Error(`GS: ${stderr.substring(0, 200)}`));
    else setTimeout(resolve, 1000);
  });
}

function printPdfSumatra(
  sumatraExe,
  filePath,
  printer,
  copies,
  ...
  exec(
    ...
    { timeout: 60000 },
    (err) => {
      if (err) reject(new Error(`Sumatra: ${err.message}`));
...
function printWordSilent(
  ...
  const rangePart =
    pageFrom && pageTo
      ? `$d.PrintOut($false,$false,3,"${pageFrom}-${pageTo}","","","",${copies})`
      : `$d.PrintOut($false,$false,0,"","","","",${copies})`;
  // Duplex via WdPrintDuplex: 0=default, 1=simplex, 2=duplex long, 3=duplex short
  const duplexLine = duplex
    ? `$d.PageSetup.TwoPagesOnOne = $false; $w.Options.PrintBackground = $false`
    : "";
  const msScript = `
$w=New-Object -ComObject Word.Application; $w.Visible=$false; $w.DisplayAlerts=0
try {
  $d=$w.Documents.Open("${esc(filePath)}",$false,$true)
  $d.PageSetup.Orientation=${orientVal}; $d.PageSetup.PaperSize=${paper.word}; $w.ActivePrinter="${printer}"
  ${duplexLine}
  ...
  return runPSScript(msScript, 120000).catch(async (err) => {
    if (!libreExe)
      throw new Error(
        "Word tidak ditemukan. Install Microsoft Word atau LibreOffice.",
      );
...
try {
  ...
  return runPSScript(msScript, 120000).catch(async (err) => {
    if (!libreExe)
      throw new Error(
        "Excel tidak ditemukan. Install Microsoft Excel atau LibreOffice.",
      );
    return printViaPdfConvert(filePath, printer, copies, libreExe);
  });
}
...
try {
  ...
  return runPSScript(msScript, 120000).catch(async (err) => {
    if (!libreExe)
      throw new Error(
        "PowerPoint tidak ditemukan. Install Microsoft PowerPoint atau LibreOffice.",
      );
    return printViaPdfConvert(filePath, printer, copies, libreExe);
  });
}
...
async function printViaPdfConvert(filePath, printer, copies, libreExe) {
  const gsExe = findGhostscript();
  if (!gsExe)
    throw new Error("Install Ghostscript untuk print via LibreOffice.");
  ...
  if (!fs.existsSync(pdfPath))
    throw new Error("LibreOffice gagal convert ke PDF.");
  try {
...
function printTxtSilent(filePath, printer, copies) {
  return runPSScript(
    `$c=Get-Content "${esc(filePath)}" -Raw
for($i=1;$i -le ${copies};$i++){$c|Out-Printer -Name "${printer}"}`,
    30000,
  );
}
...
async function printOfficeLibre(
  filePath, printer, copies, orientation,
  paperSize = "a4", pageFrom = null, pageTo = null, nup = 1, duplex = false
) {
  const libreExe = findLibreOffice();
  if (!libreExe) {
    const ext = path.extname(filePath).toUpperCase().replace(".", "");
    throw new Error(`LibreOffice tidak ditemukan. Install dari libreoffice.org untuk print ${ext}.`);
  }
  const tmpDir = os.tmpdir();
  const baseName = path.basename(filePath, path.extname(filePath));
  const outPdf = path.join(tmpDir, `lp_lo_${baseName}_${Date.now()}.pdf`);
  try {
    await runCmd(`"${libreExe}" --headless --convert-to pdf --outdir "${tmpDir}" "${filePath}"`, 120000);
    const loOut = path.join(tmpDir, baseName + ".pdf");
    if (!fs.existsSync(loOut))
      throw new Error("LibreOffice gagal mengkonversi file ke PDF.");
    fs.renameSync(loOut, outPdf);
    await printPdfSilent(outPdf, printer, copies, orientation, pageFrom, pageTo, nup, duplex, paperSize);
  } finally {
    try { fs.unlinkSync(outPdf); } catch {}
  }
}

function wrapPrintError(ext, msg) {
  if (msg && (msg.includes("Install") || msg.includes("tidak ditemukan"))) return msg;
  const hints = {
    ".pdf":  "Pastikan Ghostscript atau SumatraPDF terinstall.",
    ".doc":  "Pastikan Microsoft Office atau LibreOffice terinstall.",
    ".docx": "Pastikan Microsoft Office atau LibreOffice terinstall.",
    ".xls":  "Pastikan Microsoft Office atau LibreOffice terinstall.",
    ".xlsx": "Pastikan Microsoft Office atau LibreOffice terinstall.",
    ".ppt":  "Pastikan Microsoft Office atau LibreOffice terinstall.",
    ".pptx": "Pastikan Microsoft Office atau LibreOffice terinstall.",
    ".odt":  "Pastikan LibreOffice terinstall (libreoffice.org).",
    ".ods":  "Pastikan LibreOffice terinstall (libreoffice.org).",
    ".rtf":  "Pastikan LibreOffice terinstall (libreoffice.org).",
    ".html": "Pastikan LibreOffice terinstall (libreoffice.org).",
    ".htm":  "Pastikan LibreOffice terinstall (libreoffice.org).",
  };
  const hint = hints[ext.toLowerCase()] || "";
...
function tmp(prefix, ext) {
  return path.join(
    os.tmpdir(),
    `lp_${prefix}_${Date.now()}_${Math.random().toString(36).substr(2, 5)}.${ext}`,
  );
}
...
function runPSScript(script, timeout = 60000) {
  return new Promise((resolve, reject) => {
    ...
    exec(
      `powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "${f}"`,
      { timeout },
      (err, stdout, stderr) => {
        fs.unlink(f, () => {});
