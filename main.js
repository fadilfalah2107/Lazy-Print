...
function createTray() {
  ...
  const menu = Menu.buildFromTemplate([
    ...
    { type: "separator" },
    {
      ...
      click: () => {
        mainWindow.show();
        mainWindow.webContents.send("tray-open-files");
      },
    },
    {
      label: "Mulai Print",
      click: () => {
        mainWindow.show();
        mainWindow.webContents.send("tray-start-print");
      },
    },
    { type: "separator" },
    {
    ...
    try {
      for (const filePath of files) {
        ...
        try {
          if ([".doc", ".docx"].includes(ext)) {
            const ok =
              libreExe && (await convertWithLibre(filePath, tmpPdf, libreExe));
...
// hapus semua watcher saat aplikasi keluar
app.on("before-quit", () => {
    ...
    for (const item of printQueue) {
      ...
      try {
        ...
        results.push({
          file: filePath,
          success: false,
          error: wrapPrintError(ext, err.message),
        });
      }
    }
    return results;
...
async function printFileSilent(
  ...
  if (process.platform === "win32") {
    ...
    if ([".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx"].includes(ext)) {
      ...
      throw new Error(
        `Install LibreOffice (libreoffice.org) atau Microsoft Office untuk print ${ext}.`,
      );
    }
...
async function printOfficeLibre(
  ...
  if (!libreExe) {
    ...
    throw new Error(
      `LibreOffice tidak ditemukan. Install dari libreoffice.org untuk print ${ext}.`,
    );
  }
...
function wrapPrintError(ext, msg) {
  if (msg && (msg.includes("Install") || msg.includes("tidak ditemukan")))
    return msg;
  const hints = {
